...
public class C_Vigenere {
    public static void main(String[] args) {
        ...
        performCryptanalyticAttack(ct, 10);
    }

    /**
     * perform cryptanalytic attack on the ciphertext
     * @param ct ciphertext
     * @param keyLength length of the key
     */
    private static void performCryptanalyticAttack(String ct, int keyLength) {
    ...
    private static void countNGram(String[] broken, int nLength) {
        HashMap<String, Integer> map = new HashMap<>();
        String maxKey = "";
        int maxVal = 0;

        for (int i=0; i<broken.length; i++) {
            for (int j=0; j<broken[i].length()-nLength; j++) {
                String nGram = broken[i].substring(j, j+nLength);
                if (map.containsKey(nGram)) {
                    int newVal = map.get(nGram) + 1;
                    if (maxVal < newVal) {
                        maxKey = nGram;
                        maxVal = newVal;
                    }

                    map.put(nGram, newVal);
                } else {
                    map.put(nGram, 0);
                }
            }
